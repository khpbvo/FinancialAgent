...
from agents import Agent, ModelSettings, Runner, RunContextWrapper, RunResult, WebSearchTool
from agents.agent import StopAtTools
from agents.guardrails import Guardrail, GuardrailError
from agents.tool import ToolChoice
from agents.hooks import AgentHooks, AsyncAgentHooks, AsyncToolHook, ToolHook
...
def tool_choice_for_critical_operations(context: RunContextWrapper[RunDeps]) -> Optional[ToolChoice]:
    ...
    async def guard(self, context: RunContextWrapper[RunDeps], input: str) -> str:
        ...
        for pii_type, pattern in patterns.items():
            if re.search(pattern, input, re.IGNORECASE):
                raise GuardrailError(f"Detected potential {pii_type} in input. Please remove sensitive information.")
        ...
        for amount_str in amounts:
            try:
                ...
                if amount > 1000000:  # Flag transactions over 1M
                    raise GuardrailError(f"Transaction amount €{amount:,.2f} seems unusually high. Please verify.")
            except ValueError:
                pass
...
class FinancialAgentHooks(AsyncAgentHooks):
    ...
    async def on_run_end(self, context: RunContextWrapper[RunDeps], result: RunResult) -> None:
        ...
        print(f"   Tool calls: {len([i for i in result.new_items if hasattr(i, 'tool_name')])}")
    
    async def on_tool_start(self, context: RunContextWrapper[RunDeps], tool_name: str, args: dict) -> None:
        """Log tool execution start."""
        print(f"🔧 Executing tool: {tool_name}")
        
    async def on_tool_end(self, context: RunContextWrapper[RunDeps], tool_name: str, result: Any) -> None:
        """Log tool execution completion."""
        print(f"   ✓ {tool_name} completed")
    
    async def on_error(self, context: RunContextWrapper[RunDeps], error: Exception) -> None:
...
def extract_financial_insights(result: RunResult) -> str:
    ...
    for item in reversed(result.new_items):
        ...
        if hasattr(item, 'output') and isinstance(item.output, (FinancialSummary, BudgetAdvice, SpendingAnalysis)):
...
def build_advanced_agent(
    ...
    return Agent[RunDeps](
        ...
        tool_use_behavior=StopAtTools(stop_at_tool_names=["add_transaction", "ingest_csv"]),
        tools=tools,
