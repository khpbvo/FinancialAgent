...
from .logging_utils import LogEvent, LogEventType, LogLevel
...
class LogAnalyzer:
    ...
    def get_session_summary(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "agent_activity": dict(df[df['agent_name'].notna()]['agent_name'].value_counts()),
        ...
        if not perf_events.empty:
            summary["performance"] = {
                ...
                "metrics_recorded": list(perf_events['data'].apply(lambda x: x.get('metric_name', 'unknown')).unique())
            }
    ...
    def get_tool_performance_analysis(self) -> Dict[str, Any]:
        ...
        tool_events = self.df[
            (self.df['event_type'].isin(['tool_call_start', 'tool_call_complete', 'tool_call_error'])) &
        ...
        for tool_name in tool_events['tool_name'].unique():
            ...
            stats = {
                ...
                "success_rate": len(completions) / len(starts) if len(starts) > 0 else 0
            }
        ...
        return {
            ...
            "summary": {
                ...
                "overall_success_rate": sum(stats['successful_calls'] for stats in tool_stats.values()) / 
    ...
    def get_error_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "errors_by_agent": dict(error_events[error_events['agent_name'].notna()]['agent_name'].value_counts()),
            "errors_by_tool": dict(error_events[error_events['tool_name'].notna()]['tool_name'].value_counts()),
            "error_timeline": []
        }
        ...
        if not error_messages.empty:
            ...
            error_patterns = error_messages.apply(lambda x: str(x)[:50] + '...' if len(str(x)) > 50 else str(x))
            analysis["common_errors"] = dict(error_patterns.value_counts().head(10))
        
        return analysis
    
    def trace_execution_flow(self, session_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        key_event_types = [
            'user_input', 'agent_start', 'orchestrator_route', 'handoff_start',
            'tool_call_start', 'tool_call_complete', 'tool_call_error',
            'agent_complete', 'agent_error'
        ]
        
        flow_events = session_events[
            session_events['event_type'].isin(key_event_types)
        ].sort_values('timestamp').head(limit)
        ...
        for _, event in flow_events.iterrows():
            ...
            if event['event_type'] == 'user_input':
                ...
                if 'output' in event['data']:
                    trace_entry["tool_output"] = event['data']['output'][:200] + '...' if len(event['data']['output']) > 200 else event['data']['output']
    ...
    def get_performance_trends(self, metric_name: Optional[str] = None, 
        ...
        for metric in perf_df['metric_name'].unique():
            metric_data = perf_df[perf_df['metric_name'] == metric].sort_values('timestamp')
            ...
            trends[metric] = {
                ...
                "trend": "improving" if values.iloc[-1] < values.iloc[0] else "degrading" if values.iloc[-1] > values.iloc[0] else "stable"
            }
            
            # Time-based analysis
            if len(metric_data) >= 10:  # Need enough points for trend analysis
                recent_data = metric_data[
                    metric_data['timestamp'] >= (metric_data['timestamp'].max() - pd.Timedelta(minutes=time_window_minutes))
                ]
                if len(recent_data) > 1:
                    recent_values = recent_data['value'].dropna()
                    trends[metric]["recent_avg"] = recent_values.mean()
                    trends[metric]["recent_trend"] = "improving" if recent_values.iloc[-1] < recent_values.iloc[0] else "degrading"
    ...
    def generate_debug_report(self, session_id: Optional[str] = None, 
        ...
        if "error" not in summary:
            ...
            report_lines.append(f"Duration: {session_info['duration_minutes']:.2f} minutes")
        ...
        if "error" not in tool_perf:
            ...
            for tool, stats in tool_perf["tool_performance"].items():
                ...
                if 'avg_execution_time_ms' in stats:
                    report_lines.append(f"  Avg time: {stats['avg_execution_time_ms']:.2f}ms")
                    report_lines.append(f"  Max time: {stats['max_execution_time_ms']:.2f}ms")
            report_lines.append("")
    ...
    def get_recent_sessions(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for session_id, group in session_groups:
            session_info = {
                ...
                "agents_used": list(group[group['agent_name'].notna()]['agent_name'].unique()),
                "tools_used": list(group[group['tool_name'].notna()]['tool_name'].unique()),
                "errors": len(group[group['level'] == 'ERROR']),
                "warnings": len(group[group['level'] == 'WARNING'])
            }
            
            # Get user inputs for this session
            user_inputs = group[group['event_type'] == 'user_input']['data'].apply(
                lambda x: x.get('user_input', '') if isinstance(x, dict) else ''
            ).tolist()
            session_info["user_inputs"] = [inp for inp in user_inputs if inp][:3]  # First 3 inputs
...
def create_debug_tool():
    ...
    def analyze_debug_logs(
        ...
        if generate_report:
            ...
            result = f"Debug Log Analysis Summary\n"
            result += f"========================\n"
