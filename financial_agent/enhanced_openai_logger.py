...
class EnhancedOpenAILogger:
    ...
    def __init__(self, log_file: Optional[Path] = None, session_id: str = ""):
        ...
        self.log_file = log_file or Path(__file__).parent / "logs" / "openai_enhanced.log"
    ...
    def start_http_call(self, method: str, url: str, headers: Dict[str, Any], content: bytes) -> Optional[str]:
        ...
        try:
            ...
            request_body = {'raw_content': content.decode('utf-8', errors='ignore')[:500]}
    ...
    def complete_http_call(self, call_id: str, status_code: int, response_content: bytes, error: Optional[Exception] = None):
        ...
        if response_content and not error:
            try:
                ...
                if 'usage' in api_call.response_body:
                    api_call.token_usage = EnhancedTokenUsage.from_dict(api_call.response_body['usage'])
                    api_call.estimated_cost_usd = self._estimate_cost(api_call.model, api_call.token_usage)
                
            except Exception as parse_error:
                api_call.response_body = {
                    'raw_content': response_content.decode('utf-8', errors='ignore')[:1000],
        ...
        if api_call.token_usage:
            data.update({
                ...
                'tokens_per_second': api_call.token_usage.total_tokens / (api_call.duration_ms / 1000) if api_call.duration_ms > 0 else 0
            })
        ...
        # Console summary
        if error:
            print(f"❌ OpenAI API call failed after {api_call.duration_ms:.0f}ms: {error}")
        else:
            tokens_info = ""
            cost_info = ""
            if api_call.token_usage:
            ...
            print(f"✅ OpenAI API call completed: {api_call.model} | {api_call.duration_ms:.0f}ms{tokens_info}{cost_info}")
    ...
    def _patch_httpx(self):
        ...
        def logged_request(self, method: str, url: str, **kwargs):
            ...
            try:
                ...
                if call_id:
                    logger.complete_http_call(call_id, response.status_code, response.content)
                return response
        ...
        if not hasattr(httpx.Client, '_original_request'):
            httpx.Client._original_request = original_request
            httpx.Client.request = logged_request
        
        if not hasattr(httpx.AsyncClient, '_original_arequest'):
            httpx.AsyncClient._original_arequest = original_arequest
            httpx.AsyncClient.request = logged_arequest
    ...
    def _patch_openai_client(self):
        ...
        try:
            ...
            if not hasattr(completions.Completions, '_enhanced_original_create'):
                ...
                completions.Completions._enhanced_original_create = original_create
                completions.Completions.create = enhanced_logged_create
...
def get_enhanced_openai_logger() -> EnhancedOpenAILogger:
    ...
    if _enhanced_openai_logger is None:
        session_id = get_logger().session_id if hasattr(get_logger(), 'session_id') else ""
