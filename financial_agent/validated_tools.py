...
class TransactionAddParams(BaseModel):
    ...
    currency: str = Field(default="EUR", regex="^[A-Z]{3}$", description="3-letter currency code")
...
class BudgetCreationParams(BaseModel):
    ...
    savings_goal_percentage: float = Field(ge=0, le=100, description="Savings goal as percentage")
...
class CategoryBudget(BaseModel):
    ...
    can_reduce_by: float = Field(default=0.0, ge=0, le=100, description="Percentage that can be reduced")
...
async def add_validated_transaction(
    ctx: RunContextWrapper[RunDeps],
    transaction: TransactionAddParams
) -> str:
...
async def create_budget_validated(
    ctx: RunContextWrapper[RunDeps],
    budget_params: BudgetCreationParams
) -> str:
    ...
    try:
        ...
        savings_target = budget_params.monthly_income * (budget_params.savings_goal_percentage / 100)
...
"""
        ...
        current_spending = {row['category']: abs(row['total']) for row in cur.fetchall()}
        ...
        for expense in budget_params.fixed_expenses:
            ...
            if current > expense.amount * 1.1:  # 10% tolerance
                warnings.append(f"⚠️ {expense.category}: Currently spending €{current:.2f}, budget is €{expense.amount:.2f}")
...
async def analyze_spending_validated(
    ...
    if 'include_categories' in analysis_params:
        categories = ', '.join([f"'{c}'" for c in analysis_params['include_categories']])
        query += f" AND category IN ({categories})"
    
    if 'exclude_categories' in analysis_params:
        categories = ', '.join([f"'{c}'" for c in analysis_params['exclude_categories']])
    ...
    if output_format == 'csv':
        ...
        for row in rows:
            lines.append(f"{row['grouping']},{row['transaction_count']},{abs(row['total_amount']):.2f},{abs(row['avg_amount']):.2f}")
        return "\n".join(lines)
        ...
        for row in rows:
            pct = (abs(row['total_amount']) / total_spending * 100) if total_spending > 0 else 0
            result += f"\n{row['grouping']}:\n"
        ...
        total_spending = sum(abs(row['total_amount']) for row in rows)
        top_categories = sorted(rows, key=lambda x: abs(x['total_amount']), reverse=True)[:5]
        ...
        for row in top_categories:
            pct = (abs(row['total_amount']) / total_spending * 100) if total_spending > 0 else 0
            result += f"  • {row['grouping']}: €{abs(row['total_amount']):.2f} ({pct:.1f}%)\n"
