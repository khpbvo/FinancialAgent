...
async def analyze_investment_readiness(
    ...
    current_emergency_savings = sum(g['current_amount'] for g in emergency_goals) if emergency_goals else 0
    target_emergency_fund = monthly_expenses * emergency_fund_months
    
    results = [f"💼 Investment Readiness Assessment\n" + "=" * 50]
    ...
    results.extend([
        ...
        f"   Savings Rate: {(monthly_surplus/monthly_income*100):.1f}%" if monthly_income > 0 else "   Savings Rate: N/A",
        ""
    ])
    ...
    if emergency_fund_complete:
        results.append(f"   ✅ Emergency Fund: €{current_emergency_savings:.2f} ({emergency_fund_months} months)")
        readiness_score += 1
    else:
        shortfall = target_emergency_fund - current_emergency_savings
        results.append(f"   ❌ Emergency Fund: Need €{shortfall:.2f} more")
        results.append(f"      Current: €{current_emergency_savings:.2f}, Target: €{target_emergency_fund:.2f}")
    
    # 2. Positive cash flow
    has_positive_cashflow = monthly_surplus > 0
    if has_positive_cashflow:
        results.append(f"   ✅ Positive Cash Flow: €{monthly_surplus:.2f}/month available")
        readiness_score += 1
    else:
        results.append(f"   ❌ Negative Cash Flow: €{abs(monthly_surplus):.2f}/month deficit")
    
    # 3. High-interest debt check (simplified - would need debt tracking in real implementation)
    results.append(f"   ⚠️ High-Interest Debt: Check manually (pay off debt >{high_interest_debt_threshold}% first)")
    ...
    if stable_income:
        results.append(f"   ✅ Income Stability: {income_stability['income_months']} months of income")
        readiness_score += 1
    else:
        results.append(f"   ❌ Income Stability: Only {income_stability['income_months']} months recorded")
    ...
    if readiness_score >= 3:
        ...
        results.extend([
            "",
            f"💰 INVESTMENT AMOUNT SUGGESTIONS:",
...
async def calculate_investment_projections(
    ...
    results = [f"📈 Investment Growth Projections\n" + "=" * 50]
    ...
    if monthly_rate > 0:
        future_value = monthly_investment * ((pow(1 + monthly_rate, months) - 1) / monthly_rate)
    else:
    ...
    if real_rate > 0:
        real_future_value = monthly_investment * ((pow(1 + real_rate, months) - 1) / real_rate)
    else:
    ...
    for milestone_year in milestones:
        if milestone_year <= years:
            ...
            if monthly_rate > 0:
                milestone_value = monthly_investment * ((pow(1 + monthly_rate, milestone_months) - 1) / monthly_rate)
            else:
                milestone_value = milestone_invested
            
            results.append(f"   Year {milestone_year}: €{milestone_value:,.0f} (invested: €{milestone_invested:,.0f})")
    ...
    if conservative_rate > 0:
        conservative_value = monthly_investment * ((pow(1 + conservative_rate, months) - 1) / conservative_rate)
    else:
        conservative_value = total_invested
    
    # Optimistic scenario
    optimistic_rate = (annual_return + 2) / 100 / 12
    optimistic_value = monthly_investment * ((pow(1 + optimistic_rate, months) - 1) / optimistic_rate)
    ...
    if years >= 10:
        ...
        if monthly_rate > 0:
            delayed_future_value = monthly_investment * ((pow(1 + monthly_rate, delayed_months) - 1) / monthly_rate)
            required_monthly = future_value * monthly_rate / ((pow(1 + monthly_rate, delayed_months) - 1))
        else:
    ...
    results.extend([
        f"🏖️ RETIREMENT INCOME POTENTIAL",
        ...
        f"• Starting now vs 5 years later: €{cost_of_waiting:,.2f} difference" if years >= 10 else f"• Time in market beats timing the market",
        f"• Consistency is more important than amount"
    ])
...
async def portfolio_rebalancing_advisor(
    ...
    results = [f"⚖️ Portfolio Rebalancing Guide\n" + "=" * 50]
