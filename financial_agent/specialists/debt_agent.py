...
async def analyze_debt_situation(
    ...
    if monthly_payment_capacity is None:
        ...
        if financial_data and financial_data["avg_income"]:
            ...
            monthly_payment_capacity = max(
                (monthly_income - monthly_expenses) * 0.2, 100
            )
        else:
            # Default conservative estimate
            monthly_payment_capacity = 500.0

    # Ensure we have a positive float value
    assert monthly_payment_capacity is not None
    payment_capacity: float = float(monthly_payment_capacity)
    ...
    results.append("   Pay debts in this order:")
    ...
    results.extend(
        [
            ...
            f"   Monthly Payment: €{payment_capacity:.2f}",
        ]
    ...
    snowball_timeline = calculate_payoff_timeline(
        snowball_order, payment_capacity, "snowball"
    )
    ...
    # Strategy comparison
    interest_savings = (
        snowball_timeline["total_interest"] - avalanche_timeline["total_interest"]
    )
    ...
    if interest_savings > 0:
        results.extend(
            [
                ...
                "   Recommendation: Use AVALANCHE if motivated by numbers",
                "   Alternative: Use SNOWBALL if you need quick wins",
            ]
        )
    else:
    ...
    accelerated_timeline = calculate_payoff_timeline(
        avalanche_order, payment_capacity + extra_payment, "avalanche"
    )

    months_saved = avalanche_timeline["months"] - accelerated_timeline["months"]
    interest_saved = (
        avalanche_timeline["total_interest"] - accelerated_timeline["total_interest"]
    )
    ...
    if high_rate_debts:
        potential_savings = sum(
            ...
            * (d.interest_rate - 10)
            / 100
            / 12
            * 24  # 2 years at 10% vs current
            for d in high_rate_debts
        )
        results.extend(
            [
                "   ⚠️ High-interest debt detected!",
                "   Consider consolidation loan at ~10% APR",
        ...
        results.append(
            "   Rates are reasonable - refinancing may not help significantly"
        )
...
def calculate_payoff_timeline(
    debts: List[DebtInfo], monthly_payment: float, strategy: str
) -> Dict[str, Any]:
    ...
    if monthly_payment <= total_debt * monthly_interest_rate:
        ...
        if monthly_interest_rate > 0:
            months = -log(
                1 - (total_debt * monthly_interest_rate) / monthly_payment
            ) / log(1 + monthly_interest_rate)
            months = int(months) + 1
    ...
    if strategy == "snowball" and debts:
        ...
        first_payoff_months = max(
            1, int(smallest_debt.balance / (monthly_payment * 0.3))
        )
    else:
...
async def create_debt_freedom_plan(
    ...
    if target_date:
        ...
        results.append(
            f"🎯 Target: Debt-free by {target_date} ({months_to_target:.0f} months)"
        )
        results.append("")
    else:
    ...
    new_surplus = (
        current_surplus
        + reduction_target
        + (income_target if aggressive_mode else income_target / 2)
    )
...
async def debt_consolidation_analyzer(
    ...
    if current_monthly_rate > 0:
        current_payment = (
            ...
            * (
                ...
                * pow(1 + current_monthly_rate, current_term_months)
            )
            / (pow(1 + current_monthly_rate, current_term_months) - 1)
        )
    ...
    if consol_monthly_rate > 0:
        consol_payment = (
            ...
            * (
                consol_monthly_rate
                * pow(1 + consol_monthly_rate, consolidation_loan_term_months)
            )
            / (pow(1 + consol_monthly_rate, consolidation_loan_term_months) - 1)
        )
    ...
    if consolidation_loan_rate < current_avg_interest - 2:  # At least 2% lower
        results.extend(
            [
                ...
                (
                    f"   • Interest savings: €{abs(total_interest_diff):,.2f}"
                    if total_interest_diff < 0
                    else ""
                ),
                "   • Simplified payments (one vs multiple)",
                "   • Fixed payment amount for budgeting",
            ]
        )

        if monthly_payment_diff < 0:
            results.append(
                f"   • Lower monthly payment: €{abs(monthly_payment_diff):.2f}"
            )
        ...
        results.extend(
            [
                ...
                "   • Better to focus on paying current debts",
            ]
        )
    ...
    if monthly_payment_diff < 0:
        ...
        results.extend(
            [
                ...
                "   Ensure you can afford higher payment",
