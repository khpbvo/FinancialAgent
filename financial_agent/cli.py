...
def _ensure_session_db_schema(db_path: Path) -> None:
    ...
    try:
        ...
        cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='agent_messages'")
        if cur.fetchone():
            cur.execute("PRAGMA table_info(agent_messages)")
            cols = [row[1] for row in cur.fetchall()]
            if 'message_data' not in cols:
                try:
                    cur.execute("ALTER TABLE agent_messages ADD COLUMN message_data TEXT")
                except Exception:
                    pass
        ...
        cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='agent_sessions'")
        if cur.fetchone():
            cur.execute("PRAGMA table_info(agent_sessions)")
            cols = [row[1] for row in cur.fetchall()]
            if 'updated_at' not in cols:
                try:
                    cur.execute("ALTER TABLE agent_sessions ADD COLUMN updated_at TEXT DEFAULT (datetime('now'))")
                except Exception:
                    pass
        conn.commit()
    except Exception:
...
async def interactive_mode(agent, deps, use_session: bool = True) -> None:
    ...
    if use_session:
        ...
        try:
            ...
            try:
                backup = session_dir / f"sessions_corrupt_{int(asyncio.get_event_loop().time()*1000)}.db"
...
""")
            ...
            elif user_input.lower() in ['bills', 'bills-csv', 'bills-excel', 'bills-pdf']:
                fmt = 'csv'
            ...
            try:
                ...
                try:
                    sync_result = await Runner.run(agent, input=user_input, context=deps, session=session)
            ...
            try:
                ...
                while True:
                    ...
                    if now - last_event_time > keepalive_interval and now - last_keepalive >= keepalive_interval:
                    ...
                    try:
                        ...
                        if (asyncio.get_event_loop().time() - last_event_time) > timeout_seconds:
                            ...
                            try:
                                ...
                                if any(t in err_text for t in ["message_data", "agent_messages.role", "no such column"]):
                                    try:
                                        new_path = Path.home() / ".financial_agent" / "sessions_v2.db"
                                        session = SQLiteSession("default_session", str(new_path))
                                        sync_result = await Runner.run(agent, input=user_input, context=deps, session=session)
                                        print(f"\n📝 Response (fresh session): {sync_result.final_output}")
                                    except Exception as second_error:
                                        print(f"❌ Fresh session failed: {second_error}. Retrying without session...")
                                        nosess = await Runner.run(agent, input=user_input, context=deps)
                    ...
                    if event.type == "raw_response_event":
                        ...
                        if hasattr(event, 'data'):
                            ...
                            if isinstance(event.data, ResponseOutputItemAddedEvent):
                                ...
                                text = getattr(added, 'text', None) or getattr(getattr(added, 'item', None), 'text', None)
                                if text:
                                    any_output_streamed = True
                                    if show_progress:
                                        print("\n📝 Response: ", end="", flush=True)
                            ...
                            if hasattr(event.data, 'type'):
                                ...
                                if 'text' in event.data.type.lower():
                        ...
                        if event.item.type == "tool_call_item":
                            ...
                            if not current_message:
                                ...
                                if not message_text:
                                    ...
                                    elif hasattr(event.item, 'output') and event.item.output:
                                        message_text = event.item.output
                                    elif hasattr(event.item, 'raw_item'):
                                        # Try to get from raw_item
                                        raw = event.item.raw_item
                                        if isinstance(raw, dict):
                                            message_text = raw.get('text') or raw.get('content') or raw.get('output')
                                ...
                                if message_text:
                                    ...
                                    message_hash = hash(message_text[:100] if len(message_text) > 100 else message_text)
                ...
                if not any_output_streamed or not current_message:
                    ...
                    try:
                        ...
                        if hasattr(result, '__dict__'):
                            ...
                            for attr_name in ['_result', 'result', '_final_result', 'final_output']:
                                if hasattr(result, attr_name):
                                    attr_value = getattr(result, attr_name)
                                    if attr_value:
                                        if hasattr(attr_value, 'final_output') and attr_value.final_output:
                                            print(f"📝 Response: {attr_value.final_output}")
                                            response_found = True
                                            break
                        ...
                        if not response_found:
                            ...
                            try:
                                final_res = await Runner.run(agent, input=user_input, context=deps, session=session)
                ...
                try:
                    sync = await Runner.run(agent, input=user_input, context=deps, session=session)
...
def main() -> None:
    ...
    parser.add_argument("--export-bills", action="store_true", help="Export bills-only recurring payments")
    parser.add_argument("--bills-format", choices=["csv", "pdf", "excel", "json"], default="csv", help="Format for --export-bills")
    parser.add_argument("--monthly-cost", action="store_true", help="Print monthly cost summary (last full month)")
    parser.add_argument("--bills-only", action="store_true", help="When used with --monthly-cost, show bills-only view")
