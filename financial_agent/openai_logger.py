...
class OpenAILogger:
        ...
        if 'messages' in sanitized:
            ...
            for msg in sanitized['messages']:
                ...
                if 'content' in msg_copy and len(str(msg_copy['content'])) > 2000:
                    msg_copy['content'] = str(msg_copy['content'])[:2000] + '...[TRUNCATED]'
    ...
    def complete_api_call(self,
        ...
        token_usage = self._extract_token_usage(response_data) if response_data else TokenUsage()
        response_size = len(json.dumps(response_data, default=str)) if response_data else 0
        
        # Calculate tokens per second
        tokens_per_second = token_usage.total_tokens / (duration_ms / 1000) if duration_ms > 0 else 0
        ...
        reasoning_content = self._extract_reasoning_content(response_data) if response_data else None
...
def _patch_openai_client():
    ...
    def logged_create(self, **kwargs):
        ...
        try:
            ...
            logger.complete_api_call(call_id, response.model_dump() if hasattr(response, 'model_dump') else response.__dict__)
            return response
    ...
    try:
        ...
        # Async client  
        if hasattr(completions.AsyncCompletions, 'create') and not hasattr(completions.AsyncCompletions, '_original_acreate'):
