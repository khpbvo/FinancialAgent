...
from agents import Item, Session
...
class SessionFactory:
    ...
    def register_backend(
        cls,
        backend: StorageBackend,
        storage_class: type[BaseSessionStorage]
    ) -> None:
...
class FinancialSessionManager:
    ...
    def __init__(self, backend: StorageBackend = StorageBackend.SQLITE, **backend_kwargs):
    ...
    async def save_interaction(
        ...
        # Save user message
        await session.save_message(SessionMessage(
            role="user",
            content=user_input,
...
class RedisSessionStorage(BaseSessionStorage):
    ...
    def __init__(self, session_id: str, redis_url: str = "redis://localhost:6379", **kwargs):
        super().__init__(session_id, **kwargs)
