...
from .openai_logger import OpenAIAPICall, TokenUsage, APICallMetrics
...
class OpenAIAnalyzer:
    ...
    def load_logs(self, start_time: Optional[datetime] = None, 
    ...
    def get_cost_analysis(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            filtered_calls = [call for call in self.calls if call.session_id == session_id]
        ...
        token_usage_by_model = defaultdict(lambda: {'prompt': 0, 'completion': 0, 'total': 0})
        ...
        for call in filtered_calls:
            ...
            token_usage_by_model[call.model]['completion'] += call.token_usage.completion_tokens
        ...
        return {
            ...
            "average_cost_per_call": total_cost / successful_calls if successful_calls > 0 else 0,
    ...
    def get_performance_analysis(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        tokens_per_second = [call.metrics.tokens_per_second for call in successful_calls if call.metrics.tokens_per_second > 0]
        ...
        for model, calls_data in performance_by_model.items():
            model_stats[model] = {
                'avg_duration_ms': sum(c['duration_ms'] for c in calls_data) / len(calls_data),
                'avg_tokens_per_second': sum(c['tokens_per_second'] for c in calls_data if c['tokens_per_second'] > 0) / max(1, len([c for c in calls_data if c['tokens_per_second'] > 0])),
                'avg_tokens': sum(c['total_tokens'] for c in calls_data) / len(calls_data),
        ...
        return {
            ...
            "slowest_calls": [
                ...
                for call in sorted(successful_calls, key=lambda x: x.metrics.duration_ms, reverse=True)[:5]
            ]
        }
    
    def get_model_usage_analysis(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        reasoning_calls = [call for call in filtered_calls if 'o1' in call.model or call.reasoning_content]
        ...
        return {
            ...
            "reasoning_calls": {
                ...
                "percentage": len(reasoning_calls) / len(filtered_calls) * 100 if filtered_calls else 0,
                "models_used": list(set(call.model for call in reasoning_calls))
            },
            "tool_usage": {
                "calls_with_tools": len(tool_call_patterns),
                "avg_tools_per_call": sum(p['tool_count'] for p in tool_call_patterns) / max(1, len(tool_call_patterns))
            },
    ...
    def get_error_analysis(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": len(error_calls) / len(filtered_calls) * 100 if filtered_calls else 0,
            ...
            "recent_errors": [
                {
                    "timestamp": call.timestamp,
                    "model": call.model,
                    "error": call.error[:200] + '...' if len(call.error) > 200 else call.error
                }
                for call in sorted(error_calls, key=lambda x: x.timestamp, reverse=True)[:5]
            ]
        }
    
    def generate_openai_report(self, session_id: Optional[str] = None, 
        ...
        if "error" not in cost_analysis:
            ...
            report_lines.append(f"Successful Calls: {cost_analysis['successful_calls']}")
            report_lines.append(f"Success Rate: {cost_analysis['success_rate']:.2%}")
            report_lines.append(f"Average Cost per Call: ${cost_analysis['average_cost_per_call']:.6f}")
            report_lines.append("")
            
            if cost_analysis['costs_by_model']:
                report_lines.append("Costs by Model:")
                for model, cost in sorted(cost_analysis['costs_by_model'].items(), key=lambda x: x[1], reverse=True):
        ...
        if "error" not in perf_analysis:
            ...
            report_lines.append(f"Average Response Time: {overall['avg_duration_ms']:.0f}ms")
            report_lines.append(f"Median Response Time: {overall['median_duration_ms']:.0f}ms")
            report_lines.append(f"Average Tokens/Second: {overall['avg_tokens_per_second']:.1f}")
            report_lines.append(f"Average Tokens/Call: {overall['avg_tokens_per_call']:.0f}")
            report_lines.append("")
            
            if perf_analysis["slowest_calls"]:
                report_lines.append("Slowest API Calls:")
                for call in perf_analysis["slowest_calls"][:3]:
                    report_lines.append(f"  {call['model']}: {call['duration_ms']:.0f}ms ({call['tokens']} tokens)")
                report_lines.append("")
        ...
        if "error" not in model_analysis:
            ...
            report_lines.append(f"Reasoning Model Calls: {model_analysis['reasoning_calls']['count']} ({model_analysis['reasoning_calls']['percentage']:.1f}%)")
            report_lines.append(f"Calls with Tools: {model_analysis['tool_usage']['calls_with_tools']}")
            report_lines.append("")
    ...
    def get_recent_sessions(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for session_id, session_calls in sessions.items():
            ...
            total_cost = sum(call.metrics.cost_usd for call in session_calls if not call.error)
            total_tokens = sum(call.token_usage.total_tokens for call in session_calls if not call.error)
            ...
            session_summaries.append({
                ...
                "avg_response_time": sum(call.metrics.duration_ms for call in session_calls if not call.error) / max(1, len(session_calls) - errors)
            })
