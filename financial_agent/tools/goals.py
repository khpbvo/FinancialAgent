...
@function_tool
def check_goals(ctx: RunContextWrapper[RunDeps], include_completed: bool = False) -> str:
    ...
    if include_completed:
        ...
        cur.execute("SELECT * FROM goals WHERE status = 'active' ORDER BY created_at DESC")
    ...
    if active_goals:
        ...
        for goal in active_goals:
            ...
            results.append(f"   Progress: €{goal['current_amount']:.2f} / €{goal['target_amount']:.2f}")
            ...
            if goal['target_date']:
                ...
                if days_left > 0:
                    ...
                    results.append(f"   📅 {days_left} days left (€{daily_needed:.2f}/day needed)")
                else:
                    results.append(f"   ⚠️ Target date has passed!")
    ...
    if active_goals:
        ...
        results.append(f"   Total saved: €{total_saved:.2f} / €{total_target:.2f} ({overall_progress:.1f}%)")
