...
def check_budget(
    ...
    results.append(f"   Total Left:   €{total_remaining:.2f} ({100-total_percentage:.1f}% remaining)")
...
def suggest_budgets(ctx: RunContextWrapper[RunDeps], months_back: int = 3) -> str:
    ...
    results = [f"💡 Budget Suggestions (based on {months_back} months of data)\n" + "=" * 50]
    ...
    for data in spending_data:
        ...
        cur.execute("SELECT amount FROM budgets WHERE category = ?", (data['category'],))
        existing = cur.fetchone()
        ...
        if existing:
            ...
            if diff > 0:
                results.append(f"   ⚠️ Current budget (€{existing['amount']:.2f}) may be too low")
            else:
                results.append(f"   ✅ Current budget (€{existing['amount']:.2f}) looks good")
