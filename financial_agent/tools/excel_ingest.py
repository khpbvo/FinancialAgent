...
def detect_column_mapping(df: pd.DataFrame) -> Dict[str, str]:
    ...
    if "af bij" in columns_lower or (
        "debit" in columns_lower and "credit" in columns_lower
    ):
...
def process_excel_file(
    deps: RunDeps, excel_path: Path, sheet_name: Optional[str] = None
) -> int:
    ...
    if "date" not in mapping or "amount" not in mapping:
        ...
        if len(columns) >= 3:
            ...
            raise ValueError(
                "Unable to detect required columns (date, amount) in Excel file"
            )
    ...
    for _, row in df.iterrows():
        ...
        cur.execute(
            INSERT_TRANSACTION,
            (date, description, amount, currency, category, str(excel_path.name)),
        )
        inserted += 1
...
def ingest_excel(
    ctx: RunContextWrapper[RunDeps], path: str, sheet_name: Optional[str] = None
) -> str:
