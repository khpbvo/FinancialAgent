...
def _export_excel(transactions: List[Dict], filepath: Path, deps: RunDeps) -> str:
    ...
    for tx in transactions:
        if tx["category"] and tx["amount"] < 0:
            categories[tx["category"]] = categories.get(tx["category"], 0) + abs(
                tx["amount"]
            )
...
def _export_pdf(transactions: List[Dict], filepath: Path, deps: RunDeps) -> str:
    ...
    metadata = [
        ...
        (
            f"<b>Date Range:</b> {transactions[-1]['date']} to {transactions[0]['date']}"
            if transactions
            else ""
        ),
    ]
    ...
    for tx in transactions[:100]:  # Limit to 100 for PDF size
        desc = (
            tx["description"][:40] + "..."
            if len(tx["description"]) > 40
            else tx["description"]
        )
        tx_data.append(
            [tx["date"], desc, f"€{tx['amount']:.2f}", tx["category"] or "-"]
        )
    ...
    for i, tx in enumerate(transactions[:100], 1):
        if tx["amount"] < 0:
            tx_table.setStyle(TableStyle([("TEXTCOLOR", (2, i), (2, i), colors.red)]))
    ...
    if len(transactions) > 100:
        ...
        elements.append(
            Paragraph(
                f"<i>Note: Showing first 100 of {len(transactions)} transactions</i>",
                styles["Normal"],
            )
        )
...
def generate_tax_report(
    ctx: RunContextWrapper[RunDeps], year: Optional[int] = None, format: str = "pdf"
) -> str:
    ...
    if not year:
        year = (
            datetime.now().year - 1 if datetime.now().month < 4 else datetime.now().year
        )
...
def _generate_tax_pdf(tax_categories: Dict, year: int, filepath: Path) -> str:
    ...
    for category_name, transactions in tax_categories.items():
        ...
        # Section header
        elements.append(
            Paragraph(category_name.replace("_", " ").title(), styles["Heading1"])
        )
        elements.append(Spacer(1, 12))
        ...
        for tx in transactions[:50]:  # Limit per category
            ...
            cat_total += amount
            cat_data.append([tx["date"], tx["description"][:50], f"€{amount:.2f}"])
        ...
        if len(transactions) > 50:
            elements.append(
                Paragraph(
                    f"<i>({len(transactions)-50} more transactions not shown)</i>",
                    styles["Normal"],
                )
            )
...
def export_budget_report(
    ctx: RunContextWrapper[RunDeps], format: str = "pdf", period_days: int = 30
) -> str:
        ...
        budget_data.append(
            {
                ...
                "percentage": (
                    (spent / adjusted_budget * 100) if adjusted_budget > 0 else 0
                ),
    ...
    if format == "json":
        export_data = {
            ...
            "summary": {
                ...
                "categories_over_budget": sum(
                    1 for b in budget_data if b["status"] == "over"
                ),
            },
        }
        ...
        budget_table = Table(
            table_data,
            colWidths=[2 * inch, 1.5 * inch, 1.5 * inch, 1.5 * inch, 1.5 * inch],
        )
...
def export_recurring_payments(
    ...
    cur.execute(
        "SELECT COUNT(*) as count FROM recurring_transactions WHERE confidence >= ?",
        (min_confidence,),
    )
...
"""
    ...
    def is_credit_repayment(p) -> bool:
        ...
        return ("creditcard" in d or "credit card" in d) and (
            "incasso" in d or "afloss" in d or "accountnr" in d
        )

    if exclude_credit_repayment:
        recurring_patterns = [
            p for p in recurring_patterns if not is_credit_repayment(p)
        ]
    ...
    if bills_only:
        ...
        recurring_patterns = [
            ...
            if not is_pos_like(p)
            and (p["category"] or "").lower() not in ("fees", "cash_withdrawal")
        ]
    ...
    for tx in all_transactions:
        for pattern in recurring_patterns:
            ...
            pattern_words = (
                pattern["description_pattern"].lower().split()[:3]
            )  # First 3 words
            ...
            if matches >= len(pattern_words) * 0.7:  # 70% word match threshold
                ...
                tol = 0.2
                if bills_only and (
                    is_variable_bill(tx_desc)
                    or is_variable_bill(pattern["description_pattern"])
                ):
                    tol = 0.4
                # Check if amount is similar
                amount_diff = abs(abs(tx["amount"]) - pattern["amount"]) / max(
                    pattern["amount"], 1e-9
                )
                if amount_diff <= tol:
    ...
    if format == "pdf":
        ...
        result = _export_recurring_pdf(
            recurring_transactions, recurring_patterns, filepath, start_date, end_date
        )
    elif format == "excel":
        if not EXCEL_AVAILABLE:
            return (
...
def _export_recurring_pdf(
    ...
    for pattern, pattern_txs in sorted(
        by_pattern.items(),
    ...
    pattern_table = Table(
        pattern_data, colWidths=[2.5 * inch, 1 * inch, 1 * inch, 1 * inch, 0.7 * inch]
    )
...
def _export_recurring_excel(
    ...
    for col in ["A", "B", "C", "D", "E"]:
        ...
        ws_summary[f"{col}{row}"].fill = PatternFill(
        ...
        export_data["summary"]["patterns"].append(
            {
                ...
                "total_amount": sum(abs(tx["amount"]) for tx in pattern_txs),
                "average_amount": sum(abs(tx["amount"]) for tx in pattern_txs)
                / len(pattern_txs),
