...
def main():
    ...
    if debug_count > 0:
        ...
        if recent_sessions:
            ...
            for session in recent_sessions:
                ...
                print(f"    Events: {session['total_events']}, Errors: {session['errors']}")
                print(f"    Tools used: {', '.join(session['tools_used'][:3])}{'...' if len(session['tools_used']) > 3 else ''}")
                print()
        ...
        if "error" not in tool_perf:
            ...
            if top_tools:
                ...
                for tool, stats in top_tools:
                    print(f"    {tool}: {stats['total_calls']} calls, {stats['success_rate']:.2%} success")
    ...
    if api_count > 0:
        ...
        if "error" not in model_analysis:
            ...
            print(f"  Reasoning calls: {model_analysis['reasoning_calls']['count']} ({model_analysis['reasoning_calls']['percentage']:.1f}%)")
            print()
    else:
